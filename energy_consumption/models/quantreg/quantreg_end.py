...
def get_QuantRegEndShort_forecasts(energydf=np.nan, indexes=[47, 51, 55, 71, 75, 79], quantiles=[0.025, 0.25, 0.5, 0.75, 0.975], abs_eval=False):
    ...
    for q in quantiles:
        ...
        beta_coeff = model_temp.params
        var_to_add = beta_coeff['tavg']**2 * 4.7127 + \
            beta_coeff['wspd']**2 * 51.5082 + \
            beta_coeff['index']**2 * 26.3693
        energyforecast[f'q{q}'] = forecast_temp + \
            norm.ppf(q, loc=0) * sqrt(var_to_add)
